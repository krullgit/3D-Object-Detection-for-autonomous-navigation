...
# appends 6 leading zeros to int
#============================================================
def get_image_index_str(img_idx):
    return "{:06d}".format(img_idx)


# creates the path where to pointscloud
#============================================================
...
def get_image_path(idx, prefix, training=True, relative_path=True, exist_check=False): # TODO: exist_check is usually True but my custom dataset has no images
...
def get_velodyne_path(idx, prefix, training=True, relative_path=True, exist_check=False): # TODO: exist_check is usually True 
    
    # custom dataset parameter
    custom_dataset = True

    if custom_dataset:
        return get_kitti_info_path(idx, prefix, 'velodyne', '.pkl', training,
                                relative_path, exist_check)
    else:
...
def get_kitti_image_info(path,
                         ...
                         num_worker=8,
                         relative_path=True,
                         with_imageshape=True): # with_imageshape was true but my custom dataset has no images
    ...
    # map function that maps every image id
    def map_func(idx):


        # create image_info object that will be returned
        image_info = {'image_idx': idx, 'pointcloud_num_features': 4}
        annotations = None


        # get path of pc and image
        if velodyne:
        ...
        if with_imageshape:
            ...
            image_info['img_shape'] = np.array((800, 1280), dtype=np.int32)
        
        # get annotations and parse them
        if label_info:
        ...
        if calib:
            ...
            # create homo coords for calibrations
            if extend_matrix:
    ...
    with futures.ThreadPoolExecutor(num_worker) as executor:
        image_infos = executor.map(map_func, image_ids) # TODO
...
def add_difficulty_to_annos(info):
    ...
    # ------------------------------------------------------------------------------------------------------ 
    #  create three levels of difficulty based on (height, occlusion, truncation)
    # ------------------------------------------------------------------------------------------------------ 
...
def get_label_anno(label_path):
    ...
    #     content = []
    # else:
    # name[0], truncated(int)[1], occluded[2], alpha[3], bbox[4:8], dimensions[8:11], location[11:14], rotation_y[14]
